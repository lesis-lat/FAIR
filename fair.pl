...
sub main {
    ...
    if (!defined $opts{username} || $opts{username} eq q{}) {
        die
"Usage: perl fair.pl --username <handle> [--depth 2] [--posts 3] [--no-cache] [--suspicious-calc] [--profile-ttl-hours 24] [--posts-ttl-hours 12]\n";
    }
    ...
    print
      "Analyzing user: $username (depth=$max_depth, posts=$posts_limit)\n";
