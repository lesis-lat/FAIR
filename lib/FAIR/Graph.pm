...
Readonly my $SECONDS_PER_HOUR                 => 3600;
Readonly my $ENGAGEMENT_THRESHOLD_DENOMINATOR => 200;
Readonly my $SCORE_WEIGHT_BURST               => 0.35;
Readonly my $SCORE_WEIGHT_TEMPORAL            => 0.25;
Readonly my $SCORE_WEIGHT_ENGAGEMENT          => 0.20;
Readonly my $SCORE_WEIGHT_USERNAME            => 0.15;
Readonly my $SCORE_WEIGHT_NAME                => 0.05;
Readonly my $SCORE_NO_POSTS_ENGAGEMENT        => 0.40;
Readonly my $SCORE_NO_POSTS_USERNAME          => 0.35;
...
sub explore_users {
    ...
    my $needs_fetch  = _needs_profile_fetch($profile_data, $profile_ttl_hours,
        $posts_ttl_hours,);
...
sub _fetch_profile_data {
    ...
    for my $key (@{$api_keys}) {
        ...
        print
"[INFO] Fetching profile $username with key $fetch_attempts/$total_keys\n";
        my $fetch_ok = eval {
            ...
            if ($raw_data) {
                ...
                if ($profile_info) {
                    ...
                    {
                        my $posts =
                          fetch_posts($username, $key, $posts_limit);
                        $profile_info -> {latest_posts} = $posts;
                    }
                    _set_cache_metadata($profile_info, $profile_ttl_hours,
                        $posts_ttl_hours,);
...
sub _handle_missing_profile {
    ...
    if ($fetch_error_message ne q{}) {
        print
"[WARN] Could not fetch profile for $username: $fetch_error_message";
        return;
    }
    if ($fetch_attempts > 0) {
    ...
    for my $relation (@relations) {
        ...
        for my $related_user (@{$post -> {$relation_key} || []}) {
            if (   !defined $related_user
                || $related_user eq q{}
                || $related_user eq $username)
            {
                next;
            }
            update_graph_node($graph, $related_user,
                full_name => $related_user);
...
sub _set_cache_metadata {
    ...
    if (exists $profile_info -> {latest_posts}) {
        my $posts_expires_at =
          $now_epoch + _hours_to_seconds($posts_ttl_hours);
...
sub _upsert_interaction_edge {
    ...
    if (graph_has_edge($graph, $username, $related_user)) {
        my $edge_data =
          graph_get_edge($graph, $username, $related_user) || {};
...
sub compute_suspicious_scores {
    ...
    for my $node (graph_nodes($graph)) {
        ...
        if ($has_posts) {
            ...
            $final_score =
              $SCORE_WEIGHT_BURST * $burst_fuzzy +
        ...
        if (!$has_posts) {
            ...
            $final_score =
              $SCORE_NO_POSTS_ENGAGEMENT * $engage_fuzzy +
...
sub _parse_date {
    ...
    my $parsed_ok = eval {
        ...
        if ($value !~ /^\d+(?:[.]\d+)?$/xms) {
            ...
            $parsed =
              Time::Piece -> strptime($normalized, '%Y-%m-%d %H:%M:%S');
        }
        1;
    };
    if (!$parsed_ok) {
