...
use FAIR::Component::Graph::Connection;
use FAIR::Component::Graph::Merge;
...
sub new {
    ...
    if ($compare_with eq $username) {
        return "--compare-with must be different from --username\n";
    }

    my $cache_path = 'cache.json';
    my $left_graph_path = "graph_${username}.json";
    my $right_graph_path = "graph_${compare_with}.json";
    my $graph_path = "graph_${username}_vs_${compare_with}.json";
    my $html_path = "graph_${username}_vs_${compare_with}.html";

    push @result, "[INFO] API keys and cache loaded successfully.\n";
    push @result,
      "Analyzing profile set: $username vs $compare_with (depth=$depth, posts=$posts)\n";
    push @result,
    ...
    FAIR::Component::Profile::Build -> new(
        [
            '--cache-path'        => $cache_path,
            '--depth'             => $depth,
            '--graph-path'        => $right_graph_path,
            '--keys-path'         => 'keys.env',
            '--posts'             => $posts,
            '--posts-ttl-hours'   => $posts_ttl_hours,
            '--profile-ttl-hours' => $profile_ttl_hours,
            '--username'          => $compare_with,
            ($no_cache ? '--no-cache' : ()),
        ]
    );

    FAIR::Component::Graph::Merge -> new(
        [
            '--graph-path'       => $graph_path,
            '--left-graph-path'  => $left_graph_path,
            '--right-graph-path' => $right_graph_path,
        ]
    );

    my $connection = FAIR::Component::Graph::Connection -> new(
        [
    ...
    if ($connection -> {has_connection}) {
        push @result,
          "[INFO] Connection found between $username and $compare_with\n";
        push @result,
          '[INFO] Connection path: '
          . join(' -> ', @{$connection -> {path}})
          . "\n";
    }

    if (!$connection -> {has_connection}) {
        push @result,
          "[INFO] No connection found between $username and $compare_with\n";
    }

    push @result,
      '[INFO] Shared neighboring profiles: '
      . $connection -> {shared_count}
      . "\n";

    if ($connection -> {shared_count} > 0) {
        push @result,
          '[INFO] Shared neighboring profiles list: '
          . join(', ', @{$connection -> {shared_neighbors}})
          . "\n";
    }
